...
public class SpaceMissionGame extends JPanel implements ActionListener, KeyListener {
    ...
    // Real Astronaut object from the astronaut system - the oxygen bar
    // below is now driven by this object's actual walk()/checkOxygen()
    // logic instead of a separate hardcoded number.
    private Engineer astronaut = new Engineer("Alex");
    private SolarPanel panel = new SolarPanel();
    ...
    private void updateSpaceMission() {
        ...
        // Oxygen now comes from the real Astronaut object instead of a
        // separate hardcoded number. walk() is called periodically
        // (not every frame) so it matches the astronaut system's own
        // pacing, and its custom exception is caught here if health runs out.
        walkTimer++;
        if (walkTimer >= 90) {
            walkTimer = 0;
            try {
                astronaut.walk();
                oxygen = astronaut.getOxygen();
            } catch (AstronautDownException e) {
                stage = LOSE;
                message = "MISSION FAILED - " + e.getMessage();
            }
        }
        ...
        for (int i = 0; i < asteroidX.length; i++) {
            ...
            if (distance < asteroidSize[i] / 2.0 + 25) {
                ...
                equipmentBroken = true;
                message = "Asteroid collision! Equipment damaged - Press R to repair";
            }
        }
    ...
    private void drawAstronaut(Graphics2D g, double x, double y) {
        ...
        // Backpack (drawn behind the body)
        g.setColor(new Color(210, 215, 218));
        g.fillRoundRect(ax - 12, ay - 50, 24, 40, 8, 8);
        g.setColor(new Color(70, 190, 220));
        g.fillRect(ax - 8, ay - 44, 6, 28);
        g.fillRect(ax + 2, ay - 44, 6, 28);

        // Body suit
        g.setColor(new Color(245, 247, 248));
        g.fillRoundRect(ax - 17, ay - 48, 34, 55, 10, 10);
        g.setColor(new Color(220, 224, 226));
        g.fillRect(ax - 17, ay - 10, 34, 6);

        // Boots
        g.setColor(new Color(60, 65, 70));
        g.fillRoundRect(ax - 16, ay + 26, 14, 10, 4, 4);
        g.fillRoundRect(ax + 2, ay + 26, 14, 10, 4, 4);
        ...
        g.setColor(new Color(200, 235, 250, 180));
        g.fillOval(ax - 12, ay - 78, 12, 8);
    ...
    public void keyPressed(KeyEvent event) {
        ...
        if (key == KeyEvent.VK_R && stage == SPACE && equipmentBroken) {
            if (energy >= 15) {
                energy -= 15;
                astronaut.repairEquipment(panel);
                equipment = panel.getCondition();
                equipmentBroken = false;
                message = astronaut.getName() + " repaired the " + panel.getClass().getSimpleName();
            } else {
